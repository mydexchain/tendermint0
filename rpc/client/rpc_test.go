...
import (
	...
	"github.com/mydexchain/tendermint0/rpc/client"
	rpchttp "github.com/mydexchain/tendermint0/rpc/client/http"
	rpclocal "github.com/mydexchain/tendermint0/rpc/client/local"
	ctypes "github.com/mydexchain/tendermint0/rpc/core/types"
	rpcclient "github.com/mydexchain/tendermint0/rpc/jsonrpc/client"
	rpctest "github.com/mydexchain/tendermint0/rpc/test"
...
func TestTxSearchWithTimeout(t *testing.T) {
	...
	result, err := timeoutClient.TxSearch("app.creator='Dexi Netoko'", false, nil, nil, "asc")
	require.Nil(t, err)
...
func TestTxSearch(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		// query using a compositeKey (see kvstore application)
		result, err = c.TxSearch("app.creator='Dexi Netoko'", false, nil, nil, "asc")
		require.Nil(t, err)
		require.Greater(t, len(result.Txs), 0, "expected a lot of transactions")
		...
		result, err = c.TxSearch("app.creator='Dexi Netoko' AND tx.height<10000", true, nil, nil, "asc")
		require.Nil(t, err)
		require.Greater(t, len(result.Txs), 0, "expected a lot of transactions")

		// query a non existing tx with page 1 and txsPerPage 1
		perPage := 1
		result, err = c.TxSearch("app.creator='Dexi Netoko'", true, nil, &perPage, "asc")
		require.Nil(t, err)
		require.Len(t, result.Txs, 0)
