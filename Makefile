...
LD_FLAGS = -X github.com/mydexchain/tendermint0/version.GitCommit=`git rev-parse --short=8 HEAD`
BUILD_FLAGS = -mod=readonly -ldflags "$(LD_FLAGS)"
HTTPS_GIT := https://github.com/mydexchain/tendermint0.git
...
draw_deps:
	...
	@goviz -i github.com/mydexchain/tendermint0/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
...
format:
	...
	find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/mydexchain/tendermint0
.PHONY: format
